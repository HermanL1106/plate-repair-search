...
網頁每次開啟、重新整理、或按「重新讀取線上表」時，都會先直接讀取 Google 試算表的最新內容。如果使用者的瀏覽器或網路阻擋 Google 跨網域讀取，才會改用內嵌於 `app.js` 的備份資料。GitHub Actions 也會每天自動從 Google 試算表更新備份資料，可在 Actions 頁面手動執行 `Update sheet data`。
